        ...
        <div className="flex lg:hidden w-full flex-col bg-white">
          ...
          <div className="w-full flex flex-row items-center justify-between px-2 py-2 bg-[#F8F9FA] shrink-0 border-t border-gray-200 overflow-x-auto [&::-webkit-scrollbar]:hidden" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 8px)' }}>
            ...
            <div className="shrink-0 flex flex-row items-center justify-start gap-2">
              <button onClick={() => setShowMarksModal(true)} className="bg-gray-200 hover:bg-gray-300 text-gray-700 text-[11px] font-bold py-2 px-3 rounded-lg shadow-sm active:scale-95 transition-transform shrink-0">
                看標註
              </button>
              {MARK_OPTIONS.map(mark => (
                <button 
                  key={mark.id} 
                  onClick={() => handleToggleMark(mark.id)} 
                  className={`text-[20px] sm:text-[24px] leading-none font-bold transition-all active:scale-90 bg-transparent border-none shrink-0 ${
            ...
            <div className="shrink-0 flex items-center justify-end">
              <button onClick={handleNext} className="bg-[#3B82F6] hover:bg-blue-600 text-white font-bold h-[60px] sm:h-[65px] px-4 rounded-xl shadow-sm transition-transform active:scale-95 text-[15px] whitespace-nowrap">
